...
public class LeaveController {
    ...
    public ResponseEntity<List<LeaveResponse>> getMine() {
        return ResponseEntity.ok(leaveService.getMine());
    }

    @PostMapping("/employee/{employeeId}")
    @PreAuthorize("hasAnyRole('EMPLOYEE','HR_MANAGER')")
    public ResponseEntity<LeaveResponse> submit(@PathVariable Long employeeId,
                                                @Valid @RequestBody LeaveRequestDto dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(leaveService.submit(dto, employeeId));
    }
